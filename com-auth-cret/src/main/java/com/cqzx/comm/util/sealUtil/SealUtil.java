...
public abstract class SealUtil {
     ...
     * @throws Exception 异常
     */
    public static void buildAndStorePersonSeal(int imageSize, int lineSize, SealFont font, String addString, String fullPath) throws Exception {
     ...
     * @throws Exception 异常
     */
    public static byte[] buildBytePersonSeal(int imageSize, int lineSize, SealFont font, String addString) throws Exception {
        return buildBytes(buildPersonSeal(imageSize, lineSize, font, addString));
    }
     ...
     * @return 字节数组
     */
    public static byte[] buildByteAndStoreSeal(SealConfiguration conf) throws Exception {
...
//        	drawFont(g2d, (borderCircleWidth + INIT_BEGIN) * 2, (borderCircleHeight + 75) * 2,conf.getViceFont()); //文字横向排列
    ...
    public static BufferedImage buildPersonSeal(int imageSize, int lineSize, SealFont font, String addString) throws Exception {
        ...
        if (font.getFontText().length() == 2) {
            if (addString != null && addString.trim().length() > 0) {
                ...
                //拉伸
                BufferedImage nbi = new BufferedImage(imageSize, imageSize / 2, bi.getType());
                Graphics2D ng2d = nbi.createGraphics();
                ng2d.setPaint(Color.RED);
                ng2d.drawImage(bi, 0, 0, imageSize, imageSize / 2, null);

                //画长方形
                ng2d.setStroke(new BasicStroke(lineSize)); //设置笔画属性
                ng2d.drawRect(0, 0, imageSize, imageSize / 2);
                ng2d.dispose();
                bi = nbi;
            }
        } else if (font.getFontText().length() == 3) { //三个字
            if (addString != null && addString.trim().length() > 0) {
    ...
    private static BufferedImage drawThreeFont(BufferedImage bi, Graphics2D g2d, SealFont font, int lineSize,
        ...
        if (isWithYin) {
            g2d.drawString(font.getFontText().substring(2, 3), marginW - 4, marginH - 8);
            ...
            g2d.setFont(f);
            g2d.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
            g2d.drawString(font.getFontText().substring(0, 1), marginW, marginH += fixH - 5);
            rectangle = f.getStringBounds(font.getFontText(), context);
            marginH += Math.abs(rectangle.getHeight());
            g2d.drawString(font.getFontText().substring(1), marginW, marginH - 5);
        } else {
            g2d.drawString(font.getFontText().substring(0, 1), marginW - 12, marginH - 5);
            //拉伸
            BufferedImage nbi = new BufferedImage(imageSize, imageSize / 2, bi.getType());
            Graphics2D ng2d = nbi.createGraphics();
            ng2d.setPaint(Color.RED);
            ...
            g2d.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
            marginW += rectangle.getCenterX() * 2 + (font.getFontSpace() == null ? INIT_BEGIN : font.getFontSpace());
            g2d.drawString(font.getFontText().substring(1, 2), marginW - 17, marginH - 5);
            
            rectangle = f.getStringBounds(font.getFontText().substring(1, 2), context);
            marginW += rectangle.getCenterX() * 2 + (font.getFontSpace() == null ? INIT_BEGIN : font.getFontSpace());
            g2d.drawString(font.getFontText().substring(2, 3), marginW - 22, marginH - 5);
        }
        return bi;
    }
