...
import com.cqzx.comm.util.SendSMS;
...
@Service
@Slf4j
//TODO 登录业务，存在的问题：1 安全性，2：session共享
    ...
    public Customer loginWithPassword(String loginname, String password, String imgcode) throws UnsupportedEncodingException, NoSuchAlgorithmException {
        ...
        if(data != null){
            ...
            if(twice >= 3){
                ...
                log.debug("从redis中获取的图片验证码-->",String.valueOf(reImgCode.get("data")));

                if (!StringUtils.equals(String.valueOf(reImgCode.get("data")),imgcode)) {
                    throw new ZxException(ExceptionEnums.LOGIN_IMG_CODE);
                }else{
...
//        log.debug("两次校验的结果-->{} ---> {}",b,customer.getCustomerPassword());
        if(!b){
            if(data != null){
                //在redis中记录用户输入失败的次数
                Integer twice = Integer.parseInt(data);
                getService.putRedisString(loginname+DEGREE,String.valueOf(twice+1));
                throw new ZxException(ExceptionEnums.LOGIN_USERNAME_PASSWORD_ERROR);
            }else{
                getService.putRedisString(loginname+DEGREE,INIT_DEGREEE);
                throw new ZxException(ExceptionEnums.LOGIN_USERNAME_PASSWORD_ERROR);
            }
        }
