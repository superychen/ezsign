...
public class SealServiceImpl implements SealService {
    ...
    @Override
    public CodeEntity createCompanySealOrg(String customerId, String companyinfoName, String creditcode, String companyId) {
        byte[] companySeal = CreateSeal.companySeal(customerId);
        String fileId = uploadFile(companySeal,creditcode+".jpg");//用企业的社会信用作为印章文件名

        Sealinfo sealinfo = new Sealinfo();
        sealinfo.setSealinfoId(MD5Hash.UUIDCreate());//企业印章的唯一id
        sealinfo.setSealinfoType(2);//这里为创建企业印章，所以type为2
        sealinfo.setSealinfoUserid(companyId);//企业使用方id,默认为创建人的id
        sealinfo.setSealinfoName(companyinfoName);//企业印章名称
        sealinfo.setSealinfoFiletype("jpg");//印章图片文件格式
        sealinfo.setSealinfoImageid(fileId);//印章图片存储ID
        //获取memberinfo表中的信息
        Map<String, Object> memberMap = dbService.get("memberinfo", "memberinfo_customerid", customerId);
        //将从数据库服务的查询出来的数据转换为json字符串
        String yc = ToJson.stringToJson(String.valueOf(memberMap.get("data")));
        log.debug("yc ----> {}",yc);
        Memberinfo memberinfo = JSONObject.parseObject(yc, Memberinfo.class);
    ...
    protected Boolean insertSeal(Sealinfo sealinfo){
        //将印章信息保存到数据库中
        log.info("sealinfo保存结果{}--->",sealinfo);
