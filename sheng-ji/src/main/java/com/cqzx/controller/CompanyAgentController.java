...
public class CompanyAgentController {
    ...
    public Boolean agentAuth(@RequestParam("file") MultipartFile file,
        ...
        try {
            log.info("从前台获取的参数--->{}---{}---{}",companyinfoAgent,companyinfoAgenttele,companyinfoName);
    ...
    /**
     * 授权书接口，给管理端提供的，授权书由管理端通过之后
     * 管理端调用我的接口
     * @param companyinfoCreditcode 对应企业的社会信用号码
     */
    @PostMapping("/after/agent")
    public Boolean afterAgent(@RequestParam("companyinfoCreditcode") String companyinfoCreditcode){
        return companyAgentService.afterAgentAuth(companyinfoCreditcode);
    }
