...
public class CommException {
    ...
    public ResponseEntity<String> handlerException(HttpServletRequest request,ZxException e){
        ...
        log.info("----------访问该路径是出现异常: {}-----------",requestURI);
        //获取到自定义异常的枚举类
    ...
    @ResponseBody
    public ResponseEntity<String> handlerRuntimerException(HttpServletRequest request,RuntimeException e){
        //执行这个请求的方法
        String requestURI = request.getRequestURI();
        log.info("----------运行时抛出的异常路径是出现异常: {},信息是{}------",requestURI,e.getMessage());
