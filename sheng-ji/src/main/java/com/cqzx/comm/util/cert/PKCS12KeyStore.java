...
public class PKCS12KeyStore {
  ...
  public boolean equals(Object obj) {
    ...
    private LinkedHashMap<Object, X509CertificateObject> certs = new LinkedHashMap<Object, X509CertificateObject>();
  ...
  return key;
     ...
     */
    public X509CertificateObject[] engineGetCertificateChain(String alias) {
  KeyEntry entry = entries.get(alias.toLowerCase());
  if (entry != null) {
      if (entry.chain == null) {
    return null;
      } else {
    return (X509CertificateObject[])entry.chain.clone();
      }
  } else {
      return null;
  }
     ...
     */
    public X509CertificateObject engineGetCertificate(String alias) {
  KeyEntry entry = entries.get(alias.toLowerCase());
  if (entry != null) {
      if (entry.chain == null) {
    return null;
  ...
  if (entry != null) {
    ...
    public synchronized void engineSetKeyEntry(String alias, Key key, 
      char[] password, X509CertificateObject[] chain)
      ...
      if (chain != null) {
    	    ...
    	    if ((chain.length > 1) && (!validateChain(chain)))
    	       throw new KeyStoreException("Certificate chain is " +
    	            "not validate");
    	    entry.chain = (X509CertificateObject[])chain.clone();
  ...
  return ((byte[])key);
    ...
    public synchronized void engineSetCertificateEntry(String alias,
    		X509CertificateObject cert) throws KeyStoreException
    {
  KeyEntry entry = entries.get(alias.toLowerCase());
  if (entry != null) {
  ...
  return false;
     ...
     */
    public String engineGetCertificateAlias(X509CertificateObject cert) {
    	X509CertificateObject certElem = null;
  ...
  return ((byte[])mData);
     ...
     */
    private boolean validateChain(X509CertificateObject[] certChain) 
    {
        for (int i = 0; i < certChain.length-1; i++) {
      X500Principal issuerDN = 
    ((X509CertificateObject)certChain[i]).getIssuerX500Principal();
      X500Principal subjectDN = 
    ((X509CertificateObject)certChain[i+1]).getSubjectX500Principal();
  ...
  byte[] bagAttributes = bagAttrs.toByteArray();

        return ((byte[])bagAttributes);
    }
  ...
  for (Enumeration<String> e = entries.keys(); e.hasMoreElements(); ) {
    ...
    X509CertificateObject cert = (X509CertificateObject)entry.chain[i];
  ...
  byte[] encryptedData = encrDataContent.toByteArray();
  ...
  byte[] encrData = out.toByteArray();

  return ((byte[])encrData);
    }
  ...
  //System.out.println("count: "+safeContentsArray[0].length());
  //System.out.println("count: "+safeContentsArray[1].length());
  ...
  for (int m = 0; m < list.length; m++) {
      ...
      if (entry.keyId != null) {
    ArrayList<X509CertificateObject> chain =
        new ArrayList<X509CertificateObject>();
    X509CertificateObject cert = certs.get(new KeyId(entry.keyId));
    while (cert != null) {
        chain.add(cert);
    ...
    if (chain.size() > 0)
        entry.chain = chain.toArray(new X509CertificateObject[chain.size()]);
      }
  }
  certs.clear();
  keyList.clear();
...
//    cert = (X509CertificateObject)cf.generateCertificate
//      (new ByteArrayInputStream(certValue.getOctetString()));
//    bagItem = cert;
    
    X509CertificateObject cert;
    ByteArrayInputStream   bIn=new ByteArrayInputStream(certValue.getOctetString());
    ASN1InputStream dIn= new ASN1InputStream(bIn);;
    ASN1Sequence     seq = (ASN1Sequence)dIn.readObject();
	X509CertificateStructure   cert2 = new X509CertificateStructure(seq);
	org.bouncycastle.asn1.x509.Certificate tempcert=org.bouncycastle.asn1.x509.Certificate.getInstance(cert2);
	cert = new X509CertificateObject(tempcert);
    bagItem = cert;
    ...
    entry.alias = alias;
    entries.put(alias.toLowerCase(), entry);
      } else if (bagItem instanceof X509CertificateObject) {
    	  X509CertificateObject cert = (X509CertificateObject)bagItem;
